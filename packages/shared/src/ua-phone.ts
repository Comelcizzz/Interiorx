/** Коди мобільних операторів України (дві цифри після 380). */
...
/** Нормалізує до 12 цифр `380XXXXXXXXX` або null. */
...
export function validateUaPhoneOptional(phone: string): string | null {
	if (!phone.trim()) return null
	return validateUaPhone(phone)
}

export function isValidUaPhone(phone: string): boolean {
	return validateUaPhone(phone) === null
}

/** Для збереження / API: +380671234567 */
export function formatUaPhoneE164(phone: string): string | null {
